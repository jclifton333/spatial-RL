...
def plot_coverages(df):
  ...
  g = sns.relplot(data=df, x='bandwidth', y='coverages', hue='L', row='sigma_sq', col='backup', kind='line',
...
def summarize_coverages_at_date(date_strs, save=False):
  results = {'L': [], 'sigma_sq': [], 'bandwidth': [], 'coverages': [], 'backup': []}
  any_found = False
  ...
  for fname in os.listdir('coverages'):
    ...
    if matches_date:
      ...
      for bandwidth, bandwidth_results in f.items():
        ...
        if bandwidth_results['grid_size'] < 4000:
          ...
          results['sigma_sq'].append(1 / bandwidth_results['beta'])
    ...
    df.sort_values(by=['backup', 'sigma_sq', 'L', 'bandwidth'], inplace=True)
    print(df)
    if save:
...
if __name__ == "__main__":
  ...
  date = '210713,210714,210715,210716,210717.csv'
  summarize_coverages_at_date(date)
