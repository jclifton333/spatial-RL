...
decay<-function(x,mass,beta0,beta1,beta2,beta3){
  return(tmp=1/(1+exp(beta0 + (beta1*(x**beta2))/(mass**beta3))))
}
...
getprobs<-function(start,dist,mass,b){
  ...
  mass=mass%*%t(mass)
  tmp=decay(dist[which(start),which(!start)],mass[which(start),which(!start)],b[1],b[2],b[3],b[4])
...
objf<-function(beta, dist, mass, data, days){
  ret=0
...
getbeta<-function(dist,mass,data,start=c(5,10,1,.1),days=max(data[which(data!=Inf)])){
  ret=optim(par=start,fn=objf,dist=dist, mass=mass, data=data, days=days)
  return(ret)
}

##
getdelta<-function(beta, dist, mass, data, days=max(data[which(data!=Inf)])){
  require(mvtnorm)
  require(numDeriv)

  #Estimate Hessian at mle
  hess=hessian(objf, beta, dist=dist, mass=mass, data=data, days=days)
...
makesims<-function(data, PID, dist, pop, beta, first=0, last=157, nsims=1000, seed=802){
  set.seed(seed)
