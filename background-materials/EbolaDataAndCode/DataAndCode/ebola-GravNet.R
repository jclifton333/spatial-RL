...
decay<-function(x,mass,beta0,beta1,beta2){
  return(tmp=1/(1+exp(beta0 + (beta1*x)/(mass**beta2))))
}
...
getprobs<-function(start,dist,mass,b){
  ...
  mass=mass%*%t(mass)
  tmp=decay(dist[which(start),which(!start)],mass[which(start),which(!start)],b[1],b[2],b[3])
...
getbeta<-function(dist,mass,data,start=c(5,100,.1),days=max(data[which(data!=Inf)])){
...
makesims<-function(data, PID, pop, dist, beta, first=0, last=157, nsims=1000, seed=802){
  set.seed(seed)
