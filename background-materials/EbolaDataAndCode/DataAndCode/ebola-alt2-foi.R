...
getprobs<-function(start,dist,mass,b){
  ...
  for(i in S){
    tmp[i] <- sum((mass[I]**b[3])/(dist[I,i]**b[4]))*((b[1]*mass[i]**b[2])/(sum((mass[-i]**b[3])/(dist[-i,i]**b[4]))**b[5]))
  }
...
getbeta<-function(dist,mass,data,start=c(-67.5, 13.6, -8.8, 125.8, -52),years=max(data[which(data!=Inf)])){
