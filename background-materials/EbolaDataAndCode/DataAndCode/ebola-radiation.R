...
decay<-function(x,rad,beta0,beta1,beta2){
  return(tmp=1/(1+exp(beta0 + beta1*x + beta2*rad)))
}

#returns probabilities that each node will be infected in the next time step
getprobs<-function(start,dist,mass,b,circlemass){
  ...
  mi=mass %*% t(rep(1,length(start)))
  nj=t(mi)
  sij=circlemass

  rad=(mi*mi*nj)/((mi+sij)*(mi+nj+sij))

  tmp=decay(dist[which(start),which(!start)], rad[which(start),which(!start)], b[1],b[2],b[3])
...
simforeward<-function(dist, mass, beta, circlemass, start, first, last){
  ret=start
  for(i in first:last){
    ...
    p=getprobs((ret<i),dist,mass,beta,circlemass)
...
objf<-function(beta, dist, mass, data, circlemass, days){
  ret=0

  for(i in 1:(days)){
    p=getprobs((data<i),dist,mass,beta,circlemass)
...
getbeta<-function(dist,mass,data,circlemass,start=c(5,1,.000001),days=max(data[which(data!=Inf)])){
  ret=optim(par=start,fn=objf,dist=dist, mass=mass, data=data, circlemass=circlemass, days=days)
  return(ret)
}

##
getdelta<-function(beta, dist, mass, data, circlemass, days=max(data[which(data!=Inf)])){
  require(mvtnorm)
  require(numDeriv)

  #Estimate Hessian at mle
  hess=hessian(objf, beta, dist=dist, mass=mass, data=data, circlemass=circlemass, days=days)
...
makesims<-function(data, PID, pop, circlemass, dist, beta, first=0, last=157, nsims=1000, seed=802){
  set.seed(seed)
  ...
  for(i in 1:nsims){
    add=simforeward(dist=dist,mass=pop,beta=unlist(beta[i,]),circlemass=circlemass,start=start,first=(first+1),last=last)
    ret[,(i+1)] <- add
