...
objf<-function(beta, dist, mass, cross, data, days){
  ret=0
...
getbeta<-function(dist,mass,cross,data,start=c(5,100,.1,.1),days=max(data[which(data!=Inf)])){
  ret=optim(par=start,fn=objf,dist=dist, mass=mass, cross=cross, data=data, days=days)
  return(ret)
}

##
getdelta<-function(beta, dist, mass,cross,  data, days=max(data[which(data!=Inf)])){
  require(mvtnorm)
  require(numDeriv)

  #Estimate Hessian at mle
  hess=hessian(objf, beta, dist=dist, mass=mass, cross=cross, data=data, days=days)
...
makesims<-function(data, PID, pop, cross, dist, beta, first=0, last=157, nsims=1000, seed=802){
  set.seed(seed)
