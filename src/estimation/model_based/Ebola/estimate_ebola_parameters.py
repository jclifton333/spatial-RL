...
@njit
def negative_log_likelihood(eta, A, Y, y_next, distance_matrix, product_matrix, T, L):
  eta0 = eta[0]
...
def fit_ebola_transition_model(env):
  objective = partial(negative_log_likelihood, A=env.A, Y=env.Y, y_next=env.y, distance_matrix=env.DISTANCE_MATRIX,
                      product_matrix=env.PRODUCT_MATRIX, T=env.T, L=env.L)
