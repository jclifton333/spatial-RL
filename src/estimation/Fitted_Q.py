...
def rollout(K, gamma, env, evaluation_budget, treatment_budget, AR, rollout_feature_times):
  ...
  #Fit 1-step model
  AR.fitRegressor(env, target, True)
  one_step_predictions = AR.autologitPredictor(env.X[-1])
  true_probs = env.true_infection_probs[-1]
  r2 = 1 - np.sum((true_probs - one_step_predictions)**2) / ( np.var(one_step_predictions) * len(one_step_predictions) )
